...
class Sign_up_parent extends CI_Controller {
...
//            curl_close($ch);
            ...
            // print_r($data);
            // exit();
            $this->load->view('sign-up-parent', $data);
        } else {
            ?>
...
<script src="<?php echo base_url() ?>assets/front/js/sweetalert.min.js"></script> 
    ...
    public function otp() {
        ...
        foreach ($otp->result() as $otps) {
            ...
            if ($new_otp == $old_otp) {
                $count++;
                $otps->category = "parent";
                $this->db->insert('student_register', $otps);

                //send email
//                $to = "sales@edugatein.com";
//                $subject = "New parent User Registered";
//                $txt = "Hi Edugatein,
//The new parent user " . $otps->firstname . " has been registered.Please check the details. Email : " . $otps->email . " Mobile : " . $otps->phone;
